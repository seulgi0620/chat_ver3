						...
						if (obj instanceof String) {
									...
									if (message.equals("participent")) {
										...
										for (int j = 0; j < chatList.size(); j++) {
											for (int k = 0; k < chatList.elementAt(j).vc_chatroom_code.size(); k++) {
												if (chatroom_code
														.equals(chatList.elementAt(j).vc_chatroom_code.get(k))) {
													chatList.elementAt(j).send(Protocol.msg(chatroom_code,
															Protocol.exit_room, this.user_id, "now_exited"));
												}
											}
										}
										...
										SelectChatList scl = new SelectChatList();
										MessagesNotRead_Call mnr_c = new MessagesNotRead_Call();
										Vector<ChatroomListVO> vc = mnr_c.selectMsgCount(user_id,
												scl.selectChatList(user_id));
										send(Protocol.msg(vc));
										...
										for (int j = 0; j < chatList.size(); j++) {
											...
											for (int k = 0; k < chatList.elementAt(j).vc_chatroom_code.size(); k++) {
												if (user_id.equals(chatList.elementAt(j).user_id)) {
													...
													SelectChatList scl = new SelectChatList();
													MessagesNotRead_Call mnr_c = new MessagesNotRead_Call();
													Vector<ChatroomListVO> vc = mnr_c.selectMsgCount(
															chatList.elementAt(j).user_id,
															scl.selectChatList(chatList.elementAt(j).user_id));
													chatList.elementAt(j).send(Protocol.msg(vc));

												} else if (chatroom_code
														.equals(chatList.elementAt(j).vc_chatroom_code.get(k))) {
													chatList.elementAt(j)
															.send(Protocol.msg(chatroom_code, Protocol.invite_user,
																	chatList.elementAt(j).user_id, "now_invite"));
												}
											}
							...
							if (raw.firstElement() instanceof String) {
								...
								for (int i = 0; i < vc_str.size(); i++) {
									chatroom_code = ccr.create_chatroom(this.user_id, Protocol.enter_room,
											"´ÔÀÌ ÀÔÀåÇÏ¼Ì½À´Ï´Ù.", vc_str.get(i));
									for (int j = 0; j < chatList.size(); j++) {
										if (chatList.elementAt(j).user_id.equals(this.user_id)
												|| chatList.elementAt(j).user_id.equals(vc_str.elementAt(i))) {
											if (chatList.elementAt(j).user_id.equals(this.user_id)) {
												chatList.elementAt(j).send(Protocol.msg(chatroom_code,
														Protocol.create_room, this.user_id, "me"));
											}
